...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	% Plot erros
    drawnow % Fardin: ADDED
	for i = 1:numbars
% 		x =get(get(handles.bars(i),'children'), 'xdata'); % Fardin: Commented
% 		x = mean(x([1 3],:)); % Fardin: Commented
x = handles.bars(i).XData + handles.bars(i).XOffset; % Fardin: ADDED
